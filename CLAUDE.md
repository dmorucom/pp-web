...
- **`css/style.css`** — 디자인 시스템 전체. `:root`의 CSS 변수가 단일 진실 원본: `--ink`(텍스트), `--accent:#7b2d8e`(유일한 포인트 컬러), `--line`, `--paper` 등. 반응형 브레이크포인트 3개(1000/860/520px). 색·간격을 바꿀 땐 개별 규칙이 아니라 이 토큰을 먼저 본다.
...
- **단일 퍼플 `#7b2d8e`만** 포인트 컬러로 쓴다. 다른 강조색을 추가하지 않는다.

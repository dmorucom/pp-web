...
- **`index.html`** — 6개 섹션의 마크업: NAV / HERO / BOOKS(도서: `어휘력 팡팡 · 우리집 시리즈` 4권 + `어휘력 팡팡 · 이게 이 뜻` 신간 — 각 시리즈가 `.sec-head` + `.books-grid` 그룹) / ABOUT / CONTACT / FOOTER. `<head>`에서 외부 `css/style.css`와 Google Fonts(Noto Sans KR)를, 하단에서 `js/main.js`를 로드한다. 인라인 `<style>` 블록은 두지 않는다(요소별 `style=""` 속성은 목업에서 온 것으로 허용).
